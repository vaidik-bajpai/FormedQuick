import type { FormSchema } from "../models/forms.model.js";
import z from "zod";
        ...
        switch (f.type) {
            ...
            case "textarea": {
                ...
                if (v.minLength !== undefined)
                    s = (s as z.ZodString).min(v.minLength, { message: `${name} must be at least ${v.minLength} characters` });
                if (v.maxLength !== undefined)
                    s = (s as z.ZodString).max(v.maxLength, { message: `${name} must be at most ${v.maxLength} characters` });
                if (v.regex)
                    s = (s as z.ZodString).regex(new RegExp(v.regex), { message: `${name} is invalid` });
            ...
            case "number": {
                ...
                if (v.minLength !== undefined)
                    s = (s as z.ZodNumber).min(v.minLength, { message: `${name} must be >= ${v.minLength}` });
                if (v.maxLength !== undefined)
                    s = (s as z.ZodNumber).max(v.maxLength, { message: `${name} must be <= ${v.maxLength}` });
                if (!f.required) s = s.optional();
                break;
            }
